      ...
      >
        <div className="flex basis-full flex-col  tracking-tight text-slate-100/50 sm:basis-1/2 w-[20rem] h-[28rem] ">
          <div className="flex flex-1 w-full rounded-lg relative ">
            <Image src={employee.imgSrc} alt={""} objectFit="cover" layout="fill" />
          </div>
          <div className="m-5">
        ...
        <motion.div
        className="absolute bottom-full mb-2 left-1/2 transform -translate-x-1/2 px-3 py-2 bg-black text-white rounded-lg"
        initial="hidden"
        animate="visible"
        variants={tooltipVariants}
        transition={{ duration: 0.3 }}
      >
        <span className="text-xl">👋</span> Drag me!
      </motion.div>
      </PinContainer>
    </div>
  );
